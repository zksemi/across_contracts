...
(0, config_1.task)("enable-l1-token-across-ecosystem", "Enable a provided token across the entire ecosystem of supported chains")
    ...
    const tokens = await Promise.all(chainIds.map((chainId) => {
        ...
        if (matchedSymbol === "USDC") {
            const nativeUsdcAddress = constants_1.TOKEN_SYMBOLS_MAP.USDC.addresses[chainId];
            const bridgedUsdcAddress = constants_1.TOKEN_SYMBOLS_MAP["USDC.e"].addresses[chainId];
